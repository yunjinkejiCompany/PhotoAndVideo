...
- (void)rc_otherConfigure
{
    ...
    [_switchPosition setTitle:@"前" forState:UIControlStateNormal];
    ...
    [_torch setTitle:@"关" forState:UIControlStateNormal];
    ...
    _videoPath = [NSURL fileURLWithPath:[NSTemporaryDirectory() stringByAppendingPathComponent:@"video_tmp.mov"]];
    ...
    if(_device)
    {
        ...
        switch(_device.position)
        {
            case AVCaptureDevicePositionBack:
            {
                ...
                [btn setTitle:@"后" forState:UIControlStateNormal];
                [_torch setTitle:@"开" forState:UIControlStateNormal];
            }
                break;
            ...
            case AVCaptureDevicePositionFront:
            {
                bSwitch = YES;
                _devicePosition = AVCaptureDevicePositionBack;
                [btn setTitle:@"前" forState:UIControlStateNormal];
            }
                break;
...
- (void)rc_toolbar:(RCMediaToolbar *)toolbar didBeginWithMediaOperation:(RCMediaOperation)operation
{
    ...
    [_switchPosition setHidden: YES];
    [_torch setHidden:YES];
...
- (void)rc_toolbarDidCancel:(RCMediaToolbar *)toolbar completionHandler:(RCMediaCommonHandler)completionHandler
{
    ...
    {
        [self reStratCaptture];
        [_switchPosition setHidden: NO];
        [_torch setHidden:NO];
    ...
    if (_device) {
        ...
        switch (_device.position) {
            ...
            case AVCaptureDevicePositionFront:
            {
                ...
                _devicePosition = AVCaptureDevicePositionFront;
                _device = [self rc_deviceWithPosition:AVCaptureDevicePositionFront];
                _videoInput = [AVCaptureDeviceInput deviceInputWithDevice:_device error:nil];
                if([_session canAddInput:_videoInput])
                {
                    [_session addInput:_videoInput];
